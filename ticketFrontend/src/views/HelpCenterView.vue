...
// Dữ liệu giả lập cho toàn bộ Help Center
const helpCategories = ref([
  {
    ...
    icon: '✨',
    articles: [
      { id: 'f1', title: 'How to discover events?', content: '<p class="mb-4">You can discover events by navigating to the <strong>Discover</strong> page from the top menu. Use the search bar to find specific artists or venues, or filter by category.</p><p>We update our event list daily!</p>' },
      { id: 'f2', title: 'Booking a ticket step-by-step', content: '<p class="mb-4">Booking is simple! Just click on any event, select your ticket type, choose your seats if applicable, and proceed to checkout.</p><ul class="list-disc pl-6 space-y-2"><li>Step 1: Choose Event</li><li>Step 2: Select Quantity</li><li>Step 3: Payment</li></ul>' }
    ]
  },
  {
    id: 'security',
    ...
    icon: '🔒',
    articles: [
      { id: 's1', title: 'How to create an account?', content: '<p>Click on the <strong>Login / Register</strong> button at the top right corner. Fill in your email, password, and basic details to get started.</p>' },
      { id: 's2', title: 'How to reset my password?', content: '<p>If you forgot your password, click on "Forgot Password" at the login screen. Enter your email, and we will send you a reset link instantly.</p>' }
    ]
  },
  {
    id: 'account',
    ...
    icon: '👤',
    articles: [
      { id: 'a1', title: 'Updating profile information', content: '<p>Go to your account settings by clicking your Avatar. Here you can change your name, phone number, and avatar image.</p>' },
      { id: 'a2', title: 'Where are my tickets?', content: '<p>All your purchased tickets are safely stored in the <strong>My Tickets</strong> section. You can access them anytime, even offline, to scan the QR code at the event entrance.</p>' }
    ]
  },
  {
    id: 'privacy',
    ...
    icon: '🛡️',
    articles: [
      { id: 'p1', title: 'Data Collection Policy', content: '<p>We only collect data necessary to process your ticket orders and improve your experience. We never sell your personal information to third parties.</p>' }
    ]
  },
  {
    id: 'terms',
    ...
    icon: '📄',
    articles: [
      { id: 't1', title: 'Refund Policy', content: '<p>Tickets are generally non-refundable unless the event is canceled or rescheduled by the organizer. Please read the specific terms on each event page.</p>' }
    ]
  }
]);

// Trạng thái quản lý Sidebar và Nội dung
const expandedCategory = ref('features'); // Mặc định mở mục đầu tiên
const activeArticle = ref(helpCategories.value[0].articles[0]); // Mặc định hiển thị bài viết đầu tiên

// Hàm xử lý khi click vào 1 Category (Mục lớn)
const toggleCategory = (categoryId) => {
  if (expandedCategory.value === categoryId) {
    expandedCategory.value = null; // Nếu đang mở thì đóng lại
  } else {
    expandedCategory.value = categoryId; // Mở mục mới
  }
};

// Hàm xử lý khi click vào 1 Article (Mục con)
const selectArticle = (article) => {
  activeArticle.value = article;
  // Trên mobile, khi chọn xong bài viết, ta có thể tự động cuộn màn hình lên đầu nội dung
          ...
          <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden sticky top-24">
            ...
            <div v-for="category in helpCategories" :key="category.id" class="border-b border-slate-100 dark:border-slate-700 last:border-0">
              ...
              <div 
                ...
                class="bg-slate-50 dark:bg-slate-900/50 px-4 py-2"
              >
                <ul class="space-y-1">
...
<style scoped>
/* Hiệu ứng mờ dần khi chuyển bài viết */
.fade-enter-active,
.fade-leave-active {
